...
library(ggplot2)
library(ellipse) #library(GGally) problem with NA's
library(tidyverse)
library(vegan)
library(labdsv)
...
########### Kruskal-Wallis #############
df %>% group_by(ring) %>% do(tidy(kruskal.test(x = .$Dneophytes, g = .$activity)))

%>% get_summary_stats(Dneophytes, type = "mean_sd")
kruskal.test(Dneophytes ~ activity, data = dat)
df %>% group_by(ring) %>% do(tidy(kruskal.test(x = .$Dneophytes, g = .$activity)))

# #	Kruskal-Wallis rank sum test
#   ring  statistic p.value parameter method                      
#   <fct>     <dbl>   <dbl>     <int> <chr>                       
# 1 1        0       1              1 Kruskal-Wallis rank sum test
# 2 2        0.0767  0.782          1 Kruskal-Wallis rank sum test
# 3 3        3.30    0.0694         1 Kruskal-Wallis rank sum test
# 4 4        0.666   0.415          1 Kruskal-Wallis rank sum test
# 5 ref      2.68    0.101          1 Kruskal-Wallis rank sum test
# -> no significance
...
dfPlot$sig<-c("","","a","a","a","a","a","a","","")

  ggplot(dfPlot,aes(x=nObs,y=value,fill=activity)) + geom_bar(stat="identity")+
  ...
  labs(y = "inverse Simpson index", x = "") + # rename y-axis
    scale_fill_manual(values=rep(c("grey41", "grey79"),5))+
    facet_grid(~ring, scales="free_x")+
    scale_y_continuous(trans = my_trans( from=1),breaks=c(1,1.5,2)) +
  theme_bw() + mytheme +
  geom_text(mapping = aes( y =2.1, label = sig),size = 6,  fontface=1)
```
...
dfPlot<- dfGather %>% filter(cover!=0) %>% count(ring,species)
