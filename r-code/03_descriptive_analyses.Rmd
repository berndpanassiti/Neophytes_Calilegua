...
using("ggplot2","ellipse","magrittr","tidyverse","vegan","labdsv","broom","gridExtra","FSA")
...
# 3 Barplots - Number of neophytes and inverse Simpson per plot depending on site activity
```{r}
...
df = vegEnv %>% dplyr::select(-site_id,-plot_id)
df$ring2 = df$ring
df[which(df$ring2 %in% c(1,2,3,4)),"ring2"] = "zones 1-4"
df$ring2=as.factor(df$ring2)
df$ring2=factor(df$ring2, levels=c("zones 1-4","ref"))
df$activity=as.factor(df$activity)
df$value=df$Nneophytes
df%<>% arrange(ring2,activity) 

wilcox.test(Nneophytes ~ activity, data = df)
dat = df %>% dplyr::filter(activity=='active')
wilcox.test(Nneophytes ~ ring2, data = dat) # W = 289, p-value = 0.02779
dat = df %>% dplyr::filter(activity=='inactive')
wilcox.test(Nneophytes ~ ring2, data = dat) # W = 510, p-value = 0.001046


dfPlot <- data_summary(df, varname=c("value"),
                    groupnames=c("ring2","activity"))
dfPlot$yminSD<-dfPlot$value-dfPlot$sd
dfPlot$ymaxSD<-dfPlot$value+dfPlot$sd
dfPlot[which(dfPlot$yminSD<0),"yminSD"]<-0

dfPlot$nObs =  paste(levels(df$activity),"\n",rep(c("zones 1-4","ref"),each=2),"\n(N=",as.vector(table(df$activity,df$ring2)),")",sep="")
...
dat = df %>% dplyr::filter(activity=='active')
wilcox.test(Dneophytes ~ ring2, data = dat) #W = 71, p-value = 0.1749
dat = df %>% dplyr::filter(activity=='inactive')
wilcox.test(Dneophytes ~ ring2, data = dat) # W = 510, p-value = 0.001046
