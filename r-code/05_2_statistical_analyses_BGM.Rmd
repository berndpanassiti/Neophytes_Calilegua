...
```{r}
gbm.tune.result.1 = data.frame(matrix(0,ncol=12,nrow=5))
...
GBMindv.cv.deviance.mean = round(mean(cv.deviance.mean[1:3]),2)
#GBMindv.cv.deviance.sd  = paste("(",round(min(cv.deviance.mean[1:3]),2),")",sep="")
GBMindv.cv.deviance.range  = paste(round(range(cv.deviance.mean[1:3])[1],2),round(range(cv.deviance.mean[1:3])[2],2),sep="-")
...
GBMdeviance = cbind(mean=GBMindv.cv.deviance.mean,range=GBMindv.cv.deviance.range,
      MEM1=round(cv.deviance.mean[4],2),MEM2=round(cv.deviance.mean[5],2))
